  ...
  <div class="flex flex-row flex-1 clear">
    ...
    <div class="flex flex-column col col-9">
      ...
      <form class="flex flex-row flex-space-between" id="send-message">
        <input type="text" name="name" class="flex flex-1">
...
const showChat = async () => {
  ...
  const messages = await client.service('realestates').find({
    query: {
      $sort: { createdAt: -1 },
      $limit: 25
    }
...
addEventListener('#send-message', 'submit', async ev => {
  ...
  await client.service('realestates').create({
    name: input.value
  });
...
client.service('realestates').on('created', addMessage); //messages
